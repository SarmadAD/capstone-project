      ...
      case "POST":
        if (session) {
          ...
          if (getUserWithCurrentEmail === undefined || getUserWithCurrentEmail === null) {
            response.status(404).json({ error: "Invite not send: User with the current Email not found." });
          } else {
            const createdInvitation = await Invitation.create({requestingUserId:session.user.id, requestedUserId:getUserWithCurrentEmail.id, status:InviteStatus.requested});
